    ...
    <div class="py-12" x-data="csvUploader()">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 md:p-8">
                ...
                <p class="text-gray-600 mb-4">Arrastra y suelta tu archivo CSV o haz clic en el área para seleccionarlo. El sistema lo validará en tu navegador antes de enviarlo al servidor.</p>
...
<?php endif; ?>
                ...
                <form id="upload-form" action="<?php echo e(route('questions-upload.store')); ?>" method="POST" enctype="multipart/form-data">
                    <?php echo csrf_field(); ?>
                    ...
                    <div x-ref="dropArea"
                        ...
                        <input type="file" id="questions_file" name="questions_file" @change="handleFileSelect($event)" accept=".csv,.txt" class="hidden" x-ref="fileInput">

                        <div class="text-center">
                            <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
                                <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m-4-4h2m-4 0h-2" stroke-width="2"></path>
                            </svg>
                            <p class="mt-2 text-sm text-gray-600">
                                <span class="font-medium text-indigo-600">Haz clic para seleccionar</span> o arrastra y suelta
                            </p>
                            <p x-text="file ? file.name : 'Ningún archivo seleccionado'" class="mt-1 text-sm text-gray-500"></p>
                    ...
                    <p class="mt-2 text-xs text-gray-500">
                        ...
                        <button type="button" @click.prevent="templateModalOpen = true" class="text-indigo-600 hover:text-indigo-800 font-semibold underline">
                            Visualizar plantilla de ejemplo
                        </button>
                    </p>

                    <div class="flex items-center justify-between mt-8 border-t pt-5">
                        <a href="<?php echo e(route('dashboard')); ?>" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-blue-600 bg-gray-100 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Cancelar</a>
        ...
        <div x-show="uploadModalOpen" 
            ...
            <div @click.away="uploadModalOpen = false" 
                ...
                <div class="px-6 py-4 border-b -mx-6 -mt-6 mb-4"><h3 class="text-lg font-medium text-gray-900">Proceso de Validación de Archivo</h3></div>
                ...
                <div class="px-6 py-4 bg-gray-50 border-t -mx-6 -mb-6 mt-4 flex justify-end space-x-3"> 
                    <button type="button" x-show="errors.length > 0" @click="exportErrors()" class="inline-flex justify-center rounded-md border border-transparent bg-yellow-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2">Exportar Errores</button>
                    <button type="button" @click="uploadModalOpen = false" class="inline-flex justify-center rounded-md border border-gray-300 bg-white py-2 px-4 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Cerrar</button>
                </div>
            </div>
        </div>
        ...
        <div x-show="templateModalOpen" 
            ...
            <div @click.away="templateModalOpen = false" 
                 ...
                 class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-5xl lg:max-w-5xl w-full"> 
                
                
                <div class="flex justify-between items-center px-6 py-4 bg-gray-50 border-b">
                    <h3 class="text-lg font-medium text-gray-900">Plantilla de Ejemplo para carga de archivo con varias preguntas</h3>
                    <button @click="templateModalOpen = false" class="text-gray-400 hover:text-gray-600 p-2 rounded-full hover:bg-gray-100 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500">&times;</button>
                </div>

                
                <div class="p-6">
                    <p class="text-sm text-gray-600 mb-4">
                        Este es el formato exacto que tu archivo CSV debe tener. Las cabeceras deben coincidir. Puedes copiar este contenido y usarlo como base.
                    </p>
                    
                    
                    <div x-data="{
                        content: `<?php echo e($templateContent); ?>`,
                        resize() {
                        ...
                        <textarea x-ref="templateText" id="content" 
                            name="content" 
                            rows="15" 
                            class="mt-1 block w-full font-mono text-sm border-gray-300 rounded-md shadow-sm whitespace-pre overflow-x-auto" 
                            readonly><?php echo e(old('content', $templateContent)); ?>
                ...
                <div class="px-6 py-4 bg-gray-50 border-t flex justify-end items-center space-x-4">
                    <button type="button" x-data="{ text: 'Copiar Contenido' }" 
                            ...
                            x-text="text"></button>
                    <button type="button" @click="templateModalOpen = false" class="px-3 py-1 bg-white border border-gray-300 text-gray-700 text-xs font-semibold rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Cerrar</button>
                </div>
            </div>
        </div>
    </div>
        ...
        function csvUploader() {
            return {
                ...
                handleFileSelect(event) {
                    ...
                    // FIX para adjuntar el archivo al input oculto antes del envío del formulario
                    if (this.file) {
                ...
                renderErrors() {
                    if (this.errors.length > 0) {
                        ...
                        this.errors.forEach(error => {
                            ...
                            const truncatedContent = originalContent.length > 100 ? originalContent.substring(0, 97) + '...' : originalContent;
                            errorTable += `<tr><td class="px-6 py-4 whitespace-nowrap">${error[0]}</td><td class="px-6 py-4 whitespace-nowrap text-red-500">${error[1]}</td><td class="px-6 py-4 font-mono text-gray-700 break-words">${truncatedContent}</td></tr>`;
                        });
                        errorTable += '</tbody></table></div>';
                        this.progressHtml += errorTable;
                    } else {
                        this.progressHtml += '<p class="text-green-600 font-bold mt-4">✔️ Archivo validado con éxito. Enviando al servidor...</p>';
...
<?php endif; ?><?php /**PATH C:\Users\byron.moreano\Herd\svr-validator08102025\resources\views/questions-upload/create.blade.php ENDPATH**/ ?>