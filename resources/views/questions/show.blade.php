                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="md:col-span-1 space-y-6">
                        ...
                        <div id="validation-actions" class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow-inner">
                            ...
                            {{-- Si la key existe en .env pero el chequeo de salud falló, muestra alerta. --}}
                            @if(config('openai.api_key') && !$healthStatus['chatgpt'])
                                <div class="mb-4 p-4 text-sm text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300" role="alert">
                                    <span class="font-medium">Alerta:</span> La API Key de ChatGPT (OpenAI) no está funcionando o es inválida. El modelo ha sido desactivado temporalmente.
                                </div>
                            @endif
                            @if(config('gemini.api_key') && !$healthStatus['gemini'])
                                <div class="mb-4 p-4 text-sm text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300" role="alert">
                                    <span class="font-medium">Alerta:</span> La API Key de Gemini (Google) no está funcionando o es inválida. El modelo ha sido desactivado temporalmente.
                                </div>
                            @endif
                            ...
                            @can('submit', $question)
                                ...
                                {{-- REGLA 3: Si no hay motores activos (activos = 0), mostrar "SIN MODELOS" --}}
                                @if($activeEnginesCount === 0)
                                    <div class="p-4 text-sm font-bold text-white rounded-lg bg-red-600 dark:bg-red-700 text-center" role="alert">
                                        SIN MODELOS
                                    </div>
                                    <p class="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
                                        No hay motores de IA con una API key válida. Contacte al administrador.
                                    </p>
                                        ...
                                        <div class="space-y-4">
                                            <div>
                                                ...
                                                <select id="ai_engine" name="ai_engine" class="mt-1 block w-full input-form">
                                                    ...
                                                    {{-- REGLA 1: Listar solo motores activos y funcionales --}}
                                                    @foreach($availableEngines as $key => $name)
                                                        <option value="{{ $key }}">{{ $name }}</option>
                                                    @endforeach

                                                    {{-- REGLA 5: Mostrar Comparativa solo si hay más de 1 motor --}}
                                            ...
                                            {{-- REGLA 4: El botón solo se muestra si hay motores --}}
