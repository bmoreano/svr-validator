 ...
 */
class ValidationParserServiceCopy
{
    /**
     * Parsea una respuesta JSON de un validador AI y la guarda en la base de datos.
     */
    /**
     * Parsea y guarda las respuestas de validación de las APIs de IA.
     */
    public function saveAiValidation(Question $question, string $jsonResponse, string $engineName): bool
    {
        try {
            ...
            if (!is_array($data)) {
                throw new \InvalidArgumentException('El JSON decodificado no es un array.');
            }
            ...
            // Comprobamos si el array es asociativo (objeto único) vs. una lista de objetos.
            if ($this->is_assoc($responses)) {
                // Si es un objeto único, lo envolvemos en un array para que sea iterable.
                $responses = [$responses];
            }
            // Ahora, $responses está garantizado de ser una lista de arrays.
            ...
            DB::transaction(function () use ($question, $aiValidator, $responses, $engineName) {
                $validation = $question->validations()->create([
                    'validator_id' => $aiValidator->id, 'status' => 'completado', 'ai_engine' => $engineName,
                ]);
...
public function saveAiValidation1(Question $question, string $jsonResponse, string $engineName): bool
    {
        try {            
        ...
        logger()->info('Respuesta JSON no es un array iterable. : 44 ');
            ...
            // ==========================================================
            // La llamada a firstOrCreate es directa. No devuelve un objeto de error.
            // Si falla, lanzará una QueryException que será capturada por el bloque catch.
            // ==========================================================
            $aiValidator = User::firstOrCreate(
                ['email' => 'ai@svr.com'],
                [
    ...
    private function mapResponseItem(array $item): array
    {
        ...
        /*logger( 
            ...
            'response     =>'. $this->normalizeResponseValue($itemLowercase['response'] ?? $itemLowercase['cumple'] ?? $itemLowercase['valor'] ?? null).
            'comment      =>'. $itemLowercase['comment'] ?? $itemLowercase['justificacion'] ?? null,
        );*/
       return [
            'criterion_id' => $itemLowercase['criterion_id'] ?? $itemLowercase['id'] ?? null,
